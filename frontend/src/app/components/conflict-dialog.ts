  ...
  template: `
    @if (sync.conflicts().length) {
      ...
      <div class="sheet" role="dialog" aria-modal="true">
        ...
        <h2 class="pt-2 text-center text-title2 font-bold">{{ 'conflict.title' | translate }}</h2>
        <p class="mx-auto mt-1.5 max-w-xs text-center text-[15px] text-label-2">
          {{ 'conflict.intro' | translate }}
        </p>
          ...
          @for (c of sync.conflicts(); track c.stockId) {
            <li class="rounded-[14px] bg-fill p-3">
              ...
              <div class="mt-2 grid grid-cols-2 gap-2">
                ...
                >
                  <span class="block text-[12px] text-label-2">{{ 'conflict.mine' | translate }}</span>
                ...
                >
                  <span class="block text-[12px] text-label-2">{{ 'conflict.theirs' | translate }}</span>
